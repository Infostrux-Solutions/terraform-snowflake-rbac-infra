  ...
  dynamic_table_grants = flatten([
    for database, grants in local.databases : [
      for role in grants.roles : {
        ...
        database  = database
        privilege = sort([for p in local.permissions_yml.permissions.database[role].dynamic_tables : upper(p)])
...
resource "snowflake_grant_ownership" "dynamic_tables" {
  for_each = {
    for uni in local.dynamic_table_grants : uni.unique => uni if contains(uni.privilege, upper("ownership"))
  }
